...
public class CopyFolderRoute extends RouteBuilder
{
    @Override
    public void configure() {
        from("seda:copyFolderQueue").setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    exchange.getIn().getHeaders().put(PropertyIds.OBJECT_TYPE_ID, "cmis:folder");
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_DESTIONATION_FOLDER_PATH, ((Item) exchange.getIn().getBody()).getDestinationPath());
